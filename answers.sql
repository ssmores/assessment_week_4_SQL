...
SELECT m.name, m.brand_name, b.founded FROM models AS m RIGHT JOIN brands AS b on b.name = m.brand_name WHERE brand_name IS NOT NULL;
...
SELECT b.name, m.name, m.year, b.discontinued, b.discontinued - year AS years_until_brand_discountinued FROM models AS m LEFT JOIN brands AS b on m.brand_name = b.name WHErE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year;
...
SELECT name, year FROM models WHERE year = (SELECT founded FROM brands WHERE founded = models.year);